  ...
  return (
    <main className="mx-auto max-w-4xl space-y-14 px-6 py-12">
      <section className="animate-fade-in">
        ...
        <h1 className="text-4xl font-display font-bold tracking-tight text-gray-100">Learning &amp; Courses</h1>
        ...
        <div className="mt-6 flex flex-wrap gap-6 text-sm text-gray-500">
          <a href="#sap-certifications" className="hover:text-gray-300 transition-colors">
            <strong className="text-gray-200">{sapCerts.length}</strong> SAP certifications
          </a>
          <a href="#third-party" className="hover:text-gray-300 transition-colors">
            <strong className="text-gray-200">{thirdParty.length}</strong> third-party certificates
          </a>
          <a href="#confirmations" className="hover:text-gray-300 transition-colors">
            <strong className="text-gray-200">{confirmations.length}</strong> OpenSAP confirmations
          </a>
          <a href="#records-of-achievement" className="hover:text-gray-300 transition-colors">
            <strong className="text-gray-200">{roa.length}</strong> OpenSAP records of achievement
          </a>
        </div>
      </section>
      ...
      <section id="sap-certifications">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {sapCerts.map((certification) => (
            ...
            >
              <p className="text-sm font-medium leading-snug text-gray-200 hover:text-accent-blue transition-colors">{certification.title}</p>
              <div className="mt-2 flex items-center gap-2">
                <p className="text-xs text-gray-500">{certification.issuer}{certification.code ? ` · ${certification.code}` : ''}</p>
      ...
      <section id="third-party">
        <h2 className="mb-5 text-xs font-semibold uppercase tracking-widest text-gray-500">Third-Party Certificates</h2>
        <div className="grid gap-3 sm:grid-cols-2">
      ...
      <section id="confirmations">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {confirmations.map((certification) => (
            ...
            >
              <p className="text-sm font-medium leading-snug text-gray-200 hover:text-accent-purple transition-colors">{certification.title}</p>
      ...
      <section id="records-of-achievement">
        <h2 className="mb-5 text-xs font-semibold uppercase tracking-widest text-gray-500">Records of Achievement · OpenSAP</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          {roa.map((certification) => (
            ...
            >
              <p className="text-sm font-medium leading-snug text-gray-200 hover:text-accent-blue transition-colors">{certification.title}</p>
              <div className="mt-2 flex items-center gap-2">
                <p className="text-xs text-gray-500">Record of Achievement · OpenSAP</p>
                {certification.is_ai && <span className="rounded bg-accent-purple/15 border border-accent-purple/20 px-1.5 py-0.5 text-xs text-accent-purple">AI</span>}
              </div>
            </a>
          ))}
