  ...
  return (
    <div className="min-h-screen">
      ...
      <header className="border-b border-surface-300/30 px-6 py-3 flex items-center gap-6 text-sm sticky top-[49px] bg-surface/80 backdrop-blur-xl z-10">
        <span className="text-text-muted font-medium">SAP Profile</span>
      ...
      <div className="max-w-6xl mx-auto flex">
        ...
        <aside className="hidden lg:block w-48 shrink-0">
          <nav className="sticky top-12 pt-10 pb-6 pr-6">
            <ul className="space-y-1">
              {NAV.map((item) => (
                <li key={item.id}>
                  <a href={`#${item.id}`} className="block text-sm py-1.5 pl-3 border-l-2 border-surface-300/30 hover:border-accent-purple/60 transition-all duration-200 text-text-subtle hover:text-text-muted">
                    {item.label}
                  </a>
                </li>
              ))}
            ...
            <div className="mt-8 pt-6 border-t border-surface-300/20 space-y-2.5">
              ...
              <a href={`mailto:${profile.contact.email}`} className="block text-xs text-text-subtle hover:text-text-subtle truncate transition-colors">{profile.contact.email}</a>
              <a href={profile.contact.linkedin} target="_blank" rel="noopener noreferrer" className="block text-xs text-text-subtle hover:text-text-subtle transition-colors">LinkedIn</a>
              <a href={profile.contact.github} target="_blank" rel="noopener noreferrer" className="block text-xs text-text-subtle hover:text-text-subtle transition-colors">GitHub</a>
              <a href={profile.contact.sapCommunity} target="_blank" rel="noopener noreferrer" className="block text-xs text-text-subtle hover:text-text-subtle transition-colors">SAP Community</a>
            </div>
          </nav>
        </aside>
        ...
        <main className="flex-1 min-w-0 px-6 lg:px-10 py-10 space-y-16">
          ...
          <section id="profile" className="animate-fade-in">
            <h1 className="text-3xl font-display font-bold tracking-tight text-text">{profile.name}</h1>
            <p className="mt-2 text-base text-text-subtle">{profile.headline}</p>

            <div className="mt-3 flex flex-wrap gap-4 text-sm text-text-subtle">
              <a href={`mailto:${profile.contact.email}`} className="hover:text-accent-blue transition-colors">{profile.contact.email}</a>
              <span className="text-text-subtle">{profile.contact.phone}</span>
              ...
              {profile.proof_points.map((point) => (
                <div key={point} className="flex gap-2 text-sm">
                  ...
                  {point.toLowerCase().includes('teched speaker') ? (
                    <span className="text-text-subtle">{point.replace('TechEd speaker', '')}<a href="/blogs/blog-18-sap-teched" className="text-accent-blue hover:underline">TechEd speaker</a></span>
                  ) : (
                    <span className="text-text-subtle">{point}</span>
                  )}
                </div>
              ))}
            </div>
            ...
            {/* Areas of Expertise */}
            <div className="mt-6">
              <h2 className="text-xs font-semibold text-text-subtle uppercase tracking-[0.15em] mb-3">Areas of Expertise</h2>
              <ul className="grid sm:grid-cols-2 gap-2">
                {profile.positioning.map((item) => (
                  <li key={item} className="flex gap-2 text-sm text-text-subtle">
            ...
            <div className="mt-8 space-y-5">
              <h2 className="text-xs font-semibold text-text-subtle uppercase tracking-[0.15em]">Skills</h2>
              {skills.map((skill) => (
                <div key={skill.id}>
                  <h3 className="text-sm font-semibold text-text-subtle mb-2">{skill.category}</h3>
          ...
          <section className="glass-card p-6 border-accent-purple/20 animate-glow-pulse">
            <div className="flex flex-wrap justify-between gap-4">
              <div>
                ...
                <h2 className="text-lg font-bold text-text">SAP + AI — real delivery, not just theory</h2>
                <p className="mt-2 text-sm text-text-subtle max-w-xl leading-relaxed">
              ...
              {experience.map((item) => (
                <div key={item.id} className="border-l-2 border-surface-300/40 pl-5 hover:border-accent-blue/50 transition-colors duration-300">
                  <div className="flex flex-wrap justify-between gap-1">
                    <div>
                      <span className="font-semibold text-text">{item.company}</span>
                      {item.client && <span className="text-text-subtle text-sm"> · {item.client}</span>}
                    </div>
                    <span className="text-sm text-text-subtle shrink-0">{item.start_date} – {item.end_date}</span>
                  </div>
                  <div className="text-sm text-text-subtle mt-0.5">{item.role} · {item.location}</div>
                  <p className="mt-2 text-sm text-text-subtle leading-relaxed">{item.description}</p>
              ...
              {projects.map((project) => (
                <div key={project.id} className={`border-l-2 pl-5 transition-colors duration-300 ${project.is_ai ? 'border-accent-purple/40 hover:border-accent-purple/70' : 'border-surface-300/40 hover:border-accent-blue/50'}`}>
                  <div className="flex flex-wrap justify-between gap-2">
                    ...
                    {project.period && (
                      <span className="text-sm text-text-subtle shrink-0">{project.period}</span>
                    )}
                  </div>
                  {project.client && (
                  ...
                  <p className="mt-1 text-sm text-text-subtle leading-relaxed">{project.ai_summary ?? project.description}</p>
                  <div className="mt-2 flex flex-wrap gap-1.5">
            ...
            <div className="mt-6 grid sm:grid-cols-2 gap-3">
              {certifications.map((cert) => (
                <div key={cert.id} className="glass-card p-3 hover:border-accent-blue/30">
                  ...
                  {cert.code && <span className="text-xs text-text-subtle mt-0.5 block">{cert.code}</span>}
                </div>
              ))}
            </div>
            <div className="mt-5 flex items-center gap-3 text-sm border-t border-surface-300/20 pt-4">
              <span className="text-text-subtle">+ 69 completed courses covering SAP HANA, BTP, ABAP, AI/ML, and Fiori</span>
          ...
          <section id="blogs">
            ...
            <p className="mt-2 text-sm text-text-subtle">Thought leadership on SAP BTP, event-driven architecture, AI-assisted workflows, and integration patterns.</p>
          ...
          <section id="patent">
            ...
            {patent && (
              <div className="mt-6 glass-card p-5 border-accent-blue/20">
                <div className="flex flex-wrap justify-between gap-2">
                  <span className="text-sm font-semibold text-text">US10304013B2</span>
                ...
                <p className="mt-2 text-sm text-text-subtle leading-relaxed">{patent.description}</p>
              </div>
            )}
          </section>
            ...
            <div className="mt-6 space-y-4">
              {awards.map((award) => (
                <div key={award.id} className="flex gap-3">
                  ...
                  <div>
                    <span className="text-sm font-semibold text-text-muted">{award.title}</span>
                    {award.year && <span className="text-sm text-text-subtle"> · {award.year}</span>}
                    <p className="text-sm text-text-subtle mt-0.5">{award.description}</p>
                  </div>
                </div>
              ))}
            </div>
